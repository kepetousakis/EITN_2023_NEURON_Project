...
apply_tuned_inhibition = 1  # Apply orientation-selective inhibitory inputs (preferring the orientation orthogonal to the intended preference of the neuron)
inh_factor = 10          # Scales frequency of tuned inhibition (if enabled)
inh_offset = 123890     # Offset for RNG seed for stim-driven inhibition
...
bad_synapse_coeff = 3   # Scales frequency of synapses preferring orthogonal orientation
...
# Allocate basal tuned inhibition ============================================== !!!!!!!!
if apply_tuned_inhibition:
	tags_basal_inh = []
	events_stim_basal_inh = []
	vecstims_stim_basal_inh = []
	syn_stim_basal_inh = []
	conn_stim_basal_inh = []
	debug_stim_basal_inh = []
		
	for i,sec in enumerate(basal):
		target_mean_preference = rng_pref_mean.repick()
		if target_mean_preference < 0:
			target_mean_preference += 180
		if target_mean_preference+90 > 360: # account for the possibility of a tag being close to 360deg, so after +90 it'd be >>360deg.
			target_mean_preference -= 360
		per_orient_probs = [gauss5pdf(basal_width,x,target_mean_preference+90) for x in stims]  # offset preference by 90deg
		...
		tags_basal_inh.append(list(syn_tags))
		for j,syn in enumerate(syn_tags):
			...
			rng_t_inh.poisson((freq_stim*activation_vector[activation_idx]*inh_factor)/1000) 
			debug_stim_basal_inh.append((j,syn,activation_idx,activation_vector[activation_idx]))

			events_stim_basal_inh.append(h.Vector())
			vecstims_stim_basal_inh.append(h.VecStim())
			for time in range(0, int(h.tstop)):
				if rng_t_inh.repick() > 0:
					events_stim_basal_inh[-1].append(time)
			vecstims_stim_basal_inh[-1].delay = 500
			vecstims_stim_basal_inh[-1].play(events_stim_basal_inh[-1])
			syn_pos = rng_pos_stim.repick()   # Same location picker as excitation, but different samples
			syn_stim_basal_inh.append(h.GABAa(sec(syn_pos)))
			conn_stim_basal_inh.append(h.NetCon(vecstims_stim_basal_inh[-1], syn_stim_basal_inh[-1], -20, 0, gaba_g))
	mprint("Allocated basal (anti) stim-driven inhibition.")
	log('_'*30)
	for entry in debug_stim_basal_inh:
		log(entry)
...
# Allocate apical tuned inhibition ============================================== !!!!!!!!
if apply_tuned_inhibition:
	tags_apical_inh = []
	events_stim_apical_inh = []
	vecstims_stim_apical_inh = []
	syn_stim_apical_inh = []
	conn_stim_apical_inh = []
	debug_stim_apical_inh = []
	...
	for i,sec in enumerate(apical):
		...
		if target_mean_preference+90 > 360: # account for the possibility of a tag being close to 360deg, so after +90 it'd be >>360deg.
			target_mean_preference -= 360
		per_orient_probs = [gauss5pdf(apical_width,x,target_mean_preference+90) for x in stims]  # offset preference by 90deg
		per_orient_norm_probs = [x/np.sum(per_orient_probs) for x in per_orient_probs]  # normalization of probabilities
		syn_tags = np.random.choice(stims, apical_nsyn_bg_inh[i], p=per_orient_norm_probs)
		tags_apical_inh.append(list(syn_tags))
		for j,syn in enumerate(syn_tags):
			distance_from_tag = np.abs([x-np.abs(syn-presented_stimulus) for x in stims])
			activation_idx = np.argmin(distance_from_tag)
			rng_t_inh.poisson((freq_stim*activation_vector[activation_idx]*inh_factor)/1000)  
			debug_stim_apical_inh.append((j,syn,activation_idx,activation_vector[activation_idx]))
			# Note: stim-driven inhibition will rarely allocate synapses for orientations near the preferred one - but it STILL will, so the max firing rate will go down a bit as well.
			
			events_stim_apical_inh.append(h.Vector())
			vecstims_stim_apical_inh.append(h.VecStim())
			for time in range(0, int(h.tstop)):
				if rng_t_inh.repick() > 0:
					events_stim_apical_inh[-1].append(time)
			vecstims_stim_apical_inh[-1].delay = 500
			vecstims_stim_apical_inh[-1].play(events_stim_apical_inh[-1])
			syn_pos = rng_pos_stim.repick()   # Same location picker as excitation, but different samples
			syn_stim_apical_inh.append(h.GABAa(sec(syn_pos)))
			conn_stim_apical_inh.append(h.NetCon(vecstims_stim_apical_inh[-1], syn_stim_apical_inh[-1], -20, 0, gaba_g))
	mprint("Allocated apical (anti) stim-driven inhibition.")
	log('+'*30)
	log('Target mean preference '+f'{target_mean_preference}')
	log('Normalized probabilities:')
	log(per_orient_norm_probs)
	for entry in debug_stim_apical_inh:
		log(entry)
		

#%%====================== Allocate attention-driven apical synapses ===========================
# This exists only in case you want to "play around" with it.
# In the default model state, no synapses are explicitly allocated to this attentional "pool" of synapses,
